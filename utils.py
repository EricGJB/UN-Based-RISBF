...
#def big_scale(distances,total_num):
#    '''
#        input the distances, return big scale fading
#    '''
#    path_loss = -15.3 - 37.6*np.log10(distances)
#    shadowing = 7*np.random.randn(total_num)
#    big_scale_fading = path_loss + shadowing
#    big_scale_fading_linear = np.sqrt(np.power(10,big_scale_fading/10))
...
#直接求norm
        ...
        # 先求和再求模平方
#        reward = tf.reduce_sum(reward_vector,axis=-1)
#        reward = tf.norm(reward_vector,axis=-1)**2
...
def beamformer_LIS_NOBF(M,N,lr):
    def minus_mean_reward(y_true,y_pred):
        phase = y_pred  
        # convert to complex form
        phase = tf.cast(phase,tf.complex128)
...
def beamformer_LIS_joint(M,N,lr):
    '''
        joint optimization of theta and w
    '''
    def minus_mean_reward(y_true,y_pred):
        phase = y_pred[:,:N] 
        # convert to complex form
        ...
        reward_vector = tf.multiply(tf.matmul(tf.matmul(G,tf.linalg.diag(theta)),hr) + hd, w)
...
#    merged_inputs = BatchNormalization()(merged_inputs)
    ...
    temp = BatchNormalization()(temp)
    # output phase of θ
    out_phase_and_w = Dense(N+2*M, activation='linear')(temp)
    model = Model(inputs=merged_inputs, outputs=out_phase_and_w)
    model.compile(loss=minus_mean_reward, optimizer=Adam(lr=lr))
    model.summary()
    return model